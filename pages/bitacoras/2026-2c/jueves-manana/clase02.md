...
permalink: /bitacoras/2026-2c/jueves-manana/clase-02/
---
...
### Cliente Pesado vs Cliente Liviano
...
#### Representación: APIs vs "Páginas" Web

Ya conversamos sobre la idea de que poner "más o menos"  lógica en el cliente. Pero podemos ir un poco más allá. Recordando que en cliente-servidor el cliente pide y el servidor responde: ¿qué responde el servidor? ¿cuán inteligente es el cliente a la hora de interpretar la respuesta?

    * Si el servidor responde HTML y CSS y el navegador web los convierte en algo visual y "bonito" en un proceso que se conoce como dibujado o "renderizado" -> cliente liviano
    * Si el servidor responda un formato más orientado al procesamiento programático, usando lenguajes que no tienen una representación visual automática (JSON, XML, formatos binarios ej. protobuf, etc.) y el cliente primero lo convierte en HTML (DOM) y CSS y luego lo renderiza -> cliente pesado

Representaciones tpicas de cliente liviano:
...
Representaciones tipicas de cliente pesado:
...
```sh
101100101010110010011011
```

> Moraleja: el cliente pesado es pesado porque además de las tareas que siempre tiene que saber hacer (renderizar, que ya viene como parte del código de chrome/ff/etc) tienen que poder ejecutar un programa escrito en js que transforma la presentación programática en HTML y CSS como paso intermedio. Esa representación la tiene que programar alguien (vos, quien haya el plugin, etc)
>

> Nota al pie: las "partes" servidores que responden (dan el servicio) respuestas pensadas para ser procesadas de forma programática (JSON, etc) se los conoce como APIs.
